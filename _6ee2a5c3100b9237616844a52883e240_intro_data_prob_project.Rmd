...
**Research quesion 3:**
Is the average height of basketball players greater than that of baseball/ softball?
...
ggplot(Q1, aes(x = educa , y = n, fill= educa )) + geom_col(na.rm = T) + scale_x_discrete(label=function(x) abbreviate(x, minlength=1)) + theme_dark() + xlab( "Education") + ylab(" # of people above $75K") 
...
```{r}
Q3 <- brfss2013 %>% summarise(., exract11, htin4) %>% group_by(exract11) %>% filter(., exract11 %in% c("Basketball", "Softball/Baseball"))
